...
while (ct <= mj)	{
  ...
  ### generate nu ~ N(0, 0.05^2). 
  ### Note: modify sigma, lmd02, lmc02, a, b when necessary. Need to make sure 1)lmc02+nu_i>0 almost surely, 2)a+(b-1)^2/2=1/2
  #sigma <- 0.05/ 0.1
  sigma <- 0.1
  nu <- rnorm(n, mean=0, sd=sigma)
  
  #lmd02 = 0.25/ 0.5
  lmd02 = 0.5
  lmd = lmd02 + gm * Z + nu
  
  ### regenerate nu_i when lmc02+nu_i<0, but the chance must be close to 0
  while (sum(lmd02+nu<0)>0) {
    whc <- which(lmd02+nu<0)  
    nw <- length(whc)
    nu[whc] <- rnorm(nw, mean=0, sd=sigma)
  }
  ...
  haz2 <- function(t, x, betas) {
    if(t>0.25){
      (lmc02 + betas[["bs"]] * x[["X"]]) * exp(betas[["ts"]] * x[["Z"]]) - 2 * a * sigma^2 * t - b*x[["nu"]]
    }
    else{
      (lmc02 + betas[["bs"]] * 0) * exp(betas[["ts"]] * x[["Z"]]) - 2 * a * sigma^2 * t - b*x[["nu"]]
      
    }
  }
  
  C <- simsurv(hazard = haz2,  x = covdat, betas = betas, maxt = tau)$eventtime
  
  
  
  ### constant censoring times
  ####Censoring generate from unif dist
  C1 = runif(n,2*tau/3,tau)
  ...
  fth1=function(beth){
    ...
    for(i in 1:n){
      for (j in 1:nt){
        if(allt[j]>0.25){
          ...
          leftth1[i,j]=(0-vbar[1,j])*phihat[i,j]*exp(-beth[2]*Z[i])*dN[i,j]
    ...
    for(i in 1:n){
      ...
      for (j in 2:nt){
        ...
        else{rightth1[i,j]=1/2*((0-vbar[1,j])*phihat[i,j]*beth[1]*0+(0-vbar[1,j-1])*phihat[i,j-1]*beth[1]*0)*dallt[j]
        }
    ...
    for(i in 1:n){
      ...
      for (j in 2:nt){
        if(allt[j]>0.25){
          ...
          rightth2[i,j]=1/2*((V[2,i]-vbar[2,j])*phihat[i,j]*beth[1]*0+(V[2,i]-vbar[2,j-1])*phihat[i,j-1]*beth[1]*0)*dallt[j]
        }
      }
    }
  ...
  #or use nleqslv(c(0,0),fgm_0, method="Newton",control=list(trace=1,allowSingular=TRUE))$x
  ...
  for (i in 1:n){
    ...
    for (j in 2:nt){
      if(allt[j]>0.25){
        ...
        partlmd22[i,j]=1/2*(phihat[i,j]*hbth[ct,1]*0+phihat[i,j-1]*hbth[ct,1]*0)*dallt[j]
      }
    }
  }
  ...
  for (i in 1:n){
    for (j in 1:nt){
      ...
      if(allt[j]>0.25){
        ...
        q1part1[i,j]=(0-vbar1[1,j])*phihat[i,j]*exp(-hbth[ct,2]*Z[i])*dN[i,j]
      }
    }
  }
  ...
  for (i in 1:n){
    ...
    for (j in 2:nt){
      ...
      if(allt[j]>0.25){
        ...
        q1part2[i,j]=1/2*((0-vbar1[1,j])*phihat[i,j]*(hbth[ct,1]*0)+(0-vbar1[1,j-1])*phihat[i,j-1]*(hbth[ct,1]*0))*dallt[j]
  ...
  for (i in 1:n){
    ...
    for (j in 1:nt){
      if(allt[j]>0.25){
        ...
        q1part3[i,j]=(0-vbar1[1,j])*phihat[i,j]*partlmd1[j]
      }
    }
  }
  ...
  for (i in 1:n){
    ...
    for (j in 1:nt){
      ...
      if(allt[j]>0.25){
        ...
        q1part4[i,j]=(0-vbar1[1,j])*phihat[i,j]*partlmd2[j]
      }
    }
  }
  ...
  for (i in 1:n){
    ...
    for (j in 2:nt){
      if(allt[j]>0.25){
        ...
        q1part22[i,j]=1/2*((V[2,i]-vbar1[2,j])*phihat[i,j]*(hbth[ct,1]*0)+(V[2,i]-vbar1[2,j-1])*phihat[i,j-1]*(hbth[ct,1]*0))*dallt[j]
      }
    }
  }
  ...
  for (i in 1:n){
    for (j in 1:nt){
      if(allt[j]>0.25){
        ...
        Bpart1[i,j]=(0-vbar1[1,j])*(ETA[j]+allt[j]*Z[i])*((Omega)^(-1))*phihat[i,j]*exp(-hbth[ct,2]*Z[i])*dN[i,j]
      }
    }
  }
  ...
  for (i in 1:n){
    ...
    for (j in 2:nt){
      if(allt[j]>0.25){
        ...
        Bpart2[i,j]=1/2*((0-vbar1[1,j])*(ETA[j]+allt[j]*Z[i])*((Omega)^(-1))*phihat[i,j]*(hbth[ct,1]*0)
                         +(0-vbar1[1,j-1])*(ETA[j-1]+allt[j-1]*Z[i])*((Omega)^(-1))*phihat[i,j-1]*(hbth[ct,1]*0))*dallt[j]
      }
    }
  }
  ...
  for (i in 1:n){
    for (j in 1:nt){
      if(allt[j]>0.25){
        ...
        Bpart3[i,j]=(0-vbar1[1,j])*(ETA[j]+allt[j]*Z[i])*((Omega)^(-1))*phihat[i,j]*partlmd1[j]
      }
    }
  }
  ...
  for (i in 1:n){
    ...
    for (j in 1:nt){
      if(allt[j]>0.25){
        ...
        Bpart4[i,j]=(0-vbar1[1,j])*(ETA[j]+allt[j]*Z[i])*((Omega)^(-1))*phihat[i,j]*partlmd2[j]
      }
    }
  ...
  for (i in 1:n){
    ...
    for (j in 2:nt){
      if(allt[j]>0.25){
        ...
        Bpart22[i,j]=1/2*((V[2,i]-vbar1[2,j])*(ETA[j]+allt[j]*Z[i])*((Omega)^(-1))*phihat[i,j]*(hbth[ct,1]*0)
                          +(V[2,i]-vbar1[2,j-1])*(ETA[j-1]+allt[j-1]*Z[i])*((Omega)^(-1))*phihat[i,j-1]*(hbth[ct,1]*0) )*dallt[j]
      }
    }
  }
  ...
  for (i in 1:n){
    ...
    for (j in 2:nt){
      if(allt[j]>0.25){
        ...
        A1[i,j]=1/2*((0-xbar1[j])*phihat[i,j]*0+(0-xbar1[j-1])*phihat[i,j-1]*0)*dallt[j]
      }
    }
  }
  ...
  for (i in 1:n){
    for (j in 1:nt){
      if(allt[j]>0.25){
        ...
        A2[i,j]=(0-xbar1[j])*phihat[i,j]*exp(-hbth[ct,2]*Z[i])*Z[i]*dN[i,j]
      }
    }
  }
  ...
  for (i in 1:n){
    ...
    for (j in 2:nt){
      if(allt[j]>0.25){
        ...
        A3[i,j]=1/2*((Z[i]-zbar1[j])*phihat[i,j]*0+(Z[i]-zbar1[j-1])*phihat[i,j-1]*0)*dallt[j]
      }
    }
  }
...
save(sgth,cpth,hbth,hgm,prob,file="set3_2_2_2_200.RData")
